...
res[3:5,1] = "snps_indels"
res[3:5,2] = c("h2","fit","acc")
...
for (p in 5:974){
  ...
  idx = is.na(Y[,p])
  vc <- hiblup.vc(y = Y[,p], K = list(ksnp, kindel), X = cov[,3:6],method = "HE",
...
write.table(res,paste("rep",rep,"_fold",fold,".snps_indels.txt",sep=""),col.names = T,row.names = F,quote=F,sep="\t")
